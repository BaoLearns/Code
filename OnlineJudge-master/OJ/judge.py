...
class Judger(Daemon):
    ...
    def _run(self):
        ...
        #infile = "1 2"
        #outfile = "3"
        #tlist.append(Tester(self.id, self.lang. infile, outfile, 1, 100000000))
        over=False
        for t in tlist:
            if over:
                t.cancel()
            ...
            if t.result:
                ...
                self.__submit.save()
                print ("submit:" + str(self.__submit))
                print ("submit:" + str(self.__submit.status))

        if not over:
            print('-'*50 + "Accepted")
            self.__submit.status=0
            self.__submit.run_time = 1
            self.__submit.run_memory = 100000
            self.__submit.save()
            print(str(t))
